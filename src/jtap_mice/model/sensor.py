...
def left_right_sensor_read(mo: ModelOutput, scene_dim: jnp.ndarray):
    x = mo.x
    y = mo.y
    ...
    left_contact = x <= 0
    right_contact = x >= (scene_dim[0] - diameter)
...
left_right_sensor_readouts = jax.vmap(left_right_sensor_read, in_axes=(0, None))