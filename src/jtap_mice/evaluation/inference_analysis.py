...
def compute_weight_component_correlations(jtap_mice_data, run_idx=None):
    """
    Compute correlations between prev/incr/prop/grid weights and final weights
    across all timesteps in a batched manner.
    
    Args:
        jtap_mice_data: JTAP data object
        run_idx: Index of the run to analyze (required if jtap_mice_data has multiple runs)
    ...
    if jtap_mice_data.num_jtap_runs > 1:
        if run_idx is None:
            raise ValueError(f"Multiple JTAP runs detected ({jtap_mice_data.num_jtap_runs} runs). Please provide run_idx.")
        if run_idx >= jtap_mice_data.num_jtap_runs:
            raise ValueError(f"run_idx {run_idx} is out of range. Available runs: 0 to {jtap_mice_data.num_jtap_runs - 1}")
        
        # Extract weight data for specific run
        weight_data_prev = jtap_mice_data.inference.weight_data.prev_weights[run_idx]
        weight_data_incr = jtap_mice_data.inference.weight_data.incremental_weights[run_idx]
        weight_data_prop = jtap_mice_data.inference.weight_data.prop_weights[run_idx]
        weight_data_grid = jtap_mice_data.inference.weight_data.grid_weights[run_idx]
        weight_data_final = jtap_mice_data.inference.weight_data.final_weights[run_idx]
        max_steps = jtap_mice_data.params.max_inference_steps[run_idx]  # Integer: number of inference timesteps
    else:
        # Single run case
        weight_data_prev = jtap_mice_data.inference.weight_data.prev_weights
        weight_data_incr = jtap_mice_data.inference.weight_data.incremental_weights
        weight_data_prop = jtap_mice_data.inference.weight_data.prop_weights
        weight_data_grid = jtap_mice_data.inference.weight_data.grid_weights
        weight_data_final = jtap_mice_data.inference.weight_data.final_weights
        max_steps = jtap_mice_data.params.max_inference_steps  # Integer: number of inference timesteps
